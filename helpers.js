...
const DateHelper = (function () {
    ...
    const stringToDateWithoutTimezone = (stringDate) => {
        const pattern = /(\d{2})\/(\d{2})\/(\d{4})/;
        return new Date(stringDate.replace(pattern,'$3-$2-$1') + "T00:00:00");
    }


    const localeStringToDate = (stringDate) => {
        const pattern = /(\d{2})\/(\d{2})\/(\d{4})/;
    ...
    return {
        ...
        localeStringToDate: localeStringToDate,
        stringToDateWithoutTimezone: stringToDateWithoutTimezone
    }
})();
...
const MoneyHelper = (function () {
    ...
    const convertToNumberWithComma = money => {
        money = money.replace(".", "")
        money = money.replace(",", ".")
        money = Number(money.replace(/[^0-9.-]+/g, "")) || 0
        return money
    ...
    return {
        ...
        convertToNumberWithComma: convertToNumberWithComma
    }
})();
